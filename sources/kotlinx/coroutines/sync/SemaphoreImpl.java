...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
        ...
        while (true) {
            ...
            while (true) {
                if (segment.getId() >= access$getSEGMENT_SIZE$p && !segment.getRemoved()) {
                    obj = SegmentOrClosed.m7576constructorimpl(segment);
                    break;
                }
                Object access$getNextOrClosed = segment.getNextOrClosed();
                if (access$getNextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                    obj = SegmentOrClosed.m7576constructorimpl(ConcurrentLinkedListKt.CLOSED);
                    break;
                }
            ...
            if (SegmentOrClosed.m7581isClosedimpl(obj)) {
                break;
            }
            Segment r6 = SegmentOrClosed.m7579getSegmentimpl(obj);
            while (true) {
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m7579getSegmentimpl(obj);
