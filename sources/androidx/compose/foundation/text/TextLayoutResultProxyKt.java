...
public final class TextLayoutResultProxyKt {
    ...
    public static final long m968coerceIn3MmeM6k(long j, Rect rect) {
        float f;
        float f2;
        if (Offset.m2572getXimpl(j) < rect.getLeft()) {
            f = rect.getLeft();
        } else if (Offset.m2572getXimpl(j) > rect.getRight()) {
            f = rect.getRight();
        } else {
            f = Offset.m2572getXimpl(j);
        }
        if (Offset.m2573getYimpl(j) < rect.getTop()) {
            f2 = rect.getTop();
        } else if (Offset.m2573getYimpl(j) > rect.getBottom()) {
            f2 = rect.getBottom();
        } else {
            f2 = Offset.m2573getYimpl(j);
        }
