...
public final class Vertices {
    ...
    public final int m3254getVertexModec2xauaI() {
    ...
    private final int[] encodeColorList(List<Color> list) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m2875toArgb8_81llA(list.get(i).m2831unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> list) {
        ...
        for (int i = 0; i < size; i++) {
            long r3 = list.get(i / 2).m2582unboximpl();
            if (i % 2 == 0) {
                f = Offset.m2572getXimpl(r3);
            } else {
                f = Offset.m2573getYimpl(r3);
            }
            fArr[i] = f;
        }
        return fArr;
