...
public final class AndroidPath implements Path {
    ...
    public int m2704getFillTypeRgk1Os() {
        if (this.internalPath.getFillType() == Path.FillType.EVEN_ODD) {
            return PathFillType.Companion.m3114getEvenOddRgk1Os();
        }
        return PathFillType.Companion.m3115getNonZeroRgk1Os();
    }

    /* renamed from: setFillType-oQ8Xj4U  reason: not valid java name */
    public void m2706setFillTypeoQ8Xj4U(int i) {
        Path.FillType fillType;
        Path path = this.internalPath;
        if (PathFillType.m3110equalsimpl0(i, PathFillType.Companion.m3114getEvenOddRgk1Os())) {
    ...
    public void addRoundRect(RoundRect roundRect) {
        ...
        this.radii[0] = CornerRadius.m2547getXimpl(roundRect.m2622getTopLeftCornerRadiuskKHJgLs());
        this.radii[1] = CornerRadius.m2548getYimpl(roundRect.m2622getTopLeftCornerRadiuskKHJgLs());
        this.radii[2] = CornerRadius.m2547getXimpl(roundRect.m2623getTopRightCornerRadiuskKHJgLs());
        this.radii[3] = CornerRadius.m2548getYimpl(roundRect.m2623getTopRightCornerRadiuskKHJgLs());
        this.radii[4] = CornerRadius.m2547getXimpl(roundRect.m2621getBottomRightCornerRadiuskKHJgLs());
        this.radii[5] = CornerRadius.m2548getYimpl(roundRect.m2621getBottomRightCornerRadiuskKHJgLs());
        this.radii[6] = CornerRadius.m2547getXimpl(roundRect.m2620getBottomLeftCornerRadiuskKHJgLs());
        this.radii[7] = CornerRadius.m2548getYimpl(roundRect.m2620getBottomLeftCornerRadiuskKHJgLs());
    ...
    public void m2707translatek4lQ0M(long j) {
        this.mMatrix.reset();
        this.mMatrix.setTranslate(Offset.m2572getXimpl(j), Offset.m2573getYimpl(j));
    ...
    public boolean m2705opN5in7k0(Path path, Path path2, int i) {
        Path.Op op;
        Intrinsics.checkNotNullParameter(path, "path1");
        Intrinsics.checkNotNullParameter(path2, "path2");
        if (PathOperation.m3121equalsimpl0(i, PathOperation.Companion.m3125getDifferenceb3I0S0c())) {
            op = Path.Op.DIFFERENCE;
        } else if (PathOperation.m3121equalsimpl0(i, PathOperation.Companion.m3126getIntersectb3I0S0c())) {
            op = Path.Op.INTERSECT;
        } else if (PathOperation.m3121equalsimpl0(i, PathOperation.Companion.m3127getReverseDifferenceb3I0S0c())) {
            op = Path.Op.REVERSE_DIFFERENCE;
        } else if (PathOperation.m3121equalsimpl0(i, PathOperation.Companion.m3128getUnionb3I0S0c())) {
            op = Path.Op.UNION;
        } else {
            op = Path.Op.XOR;
        }
    ...
    public void m2703addPathUv8p0NA(Path path, long j) {
        Intrinsics.checkNotNullParameter(path, "path");
        Path path2 = this.internalPath;
        if (path instanceof AndroidPath) {
            path2.addPath(((AndroidPath) path).getInternalPath(), Offset.m2572getXimpl(j), Offset.m2573getYimpl(j));
            return;
        }
        throw new UnsupportedOperationException("Unable to obtain android.graphics.Path");
    }
