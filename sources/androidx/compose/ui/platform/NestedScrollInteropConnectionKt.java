...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m4921toOffsetUv8p0NA(int[] iArr, long j) {
        float f;
        float f2;
        if (Offset.m2572getXimpl(j) >= 0.0f) {
            f = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m2572getXimpl(j));
        } else {
            f = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m2572getXimpl(j));
        }
        if (Offset.m2573getYimpl(j) >= 0.0f) {
            f2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m2573getYimpl(j));
        } else {
            f2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m2573getYimpl(j));
        }
    ...
    public static final int m4922toViewTypeGyEprt8(int i) {
        return NestedScrollSource.m4218equalsimpl0(i, NestedScrollSource.Companion.m4223getDragWNlRxjI()) ^ true ? 1 : 0;
    }

    /* access modifiers changed from: private */
    /* renamed from: getScrollAxes-k-4lQ0M  reason: not valid java name */
    public static final int m4920getScrollAxesk4lQ0M(long j) {
        int i = Math.abs(Offset.m2572getXimpl(j)) >= 0.5f ? 1 : 0;
        return Math.abs(Offset.m2573getYimpl(j)) >= 0.5f ? i | 2 : i;
    }
