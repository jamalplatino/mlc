...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> state, State<RippleAlpha> state2, RippleContainer rippleContainer2) {
        ...
        this.rippleSize = Size.Companion.m2650getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        this.rippleSize = contentDrawScope.m3429getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m1100getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.m3429getSizeNHjbRc()));
        } else {
            i = contentDrawScope.m5601roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long r5 = this.color.getValue().m2831unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        DrawScope drawScope = contentDrawScope;
        m1105drawStateLayerH2RKhps(drawScope, this.radius, r5);
        Canvas canvas = drawScope.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1104updateRipplePropertiesbiQXAtU(contentDrawScope.m3429getSizeNHjbRc(), this.rippleRadius, r5, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press press, CoroutineScope coroutineScope) {
        ...
        rippleHostView.m1103addRippleKOepWvA(press, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m2831unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
