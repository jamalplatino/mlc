...
final class RichTooltipPositionProvider implements PopupPositionProvider {
    ...
    public long m1516calculatePositionllwVHH4(IntRect intRect, long j, LayoutDirection layoutDirection, long j2) {
        Intrinsics.checkNotNullParameter(intRect, "anchorBounds");
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        int right = intRect.getRight();
        if (IntSize.m5786getWidthimpl(j2) + right > IntSize.m5786getWidthimpl(j) && (right = intRect.getLeft() - IntSize.m5786getWidthimpl(j2)) < 0) {
            right = intRect.getLeft() + ((intRect.getWidth() - IntSize.m5786getWidthimpl(j2)) / 2);
        }
        int top = (intRect.getTop() - IntSize.m5785getHeightimpl(j2)) - this.tooltipAnchorPadding;
        if (top < 0) {
